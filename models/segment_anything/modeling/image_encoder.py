...
class ImageEncoderViT(nn.Module):
    def __init__(self, img_size: int = 1024, patch_size: int = 16, in_chans: int = 3,
        ...
        self.global_index = global_attn_indexes
    ...
    def forward(self, x: torch.Tensor, prompt_tokens: torch.Tensor = None) -> torch.Tensor:
        ...
        if self.window_size > 0:
            ...
            if prompt_tokens is not None:
                windows_num = x.shape[0] // prompt_tokens.shape[0]
                prompt_tokens = prompt_tokens.repeat_interleave(windows_num, dim=0)
        x = self.attn(x, prompt_tokens)
        if self.window_size > 0:
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prompt_tokens: torch.Tensor) -> torch.Tensor:
        B, H, W, _ = x.shape
        token_num = 0 if prompt_tokens is None else prompt_tokens.size(1)
        if token_num > 0:
            x = torch.cat((prompt_tokens.expand(B, -1, -1), x.reshape(B, H * W, -1)), dim=1)
        qkv = self.qkv(x).reshape(B, token_num + H * W, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.reshape(3, B * self.num_heads, token_num + H * W, -1).unbind(0)
        attn = (q * self.scale) @ k.transpose(-2, -1)
        if self.use_rel_pos:
            attn = add_decomposed_rel_pos(attn, q, self.rel_pos_h, self.rel_pos_w, (H, W), (H, W), token_num)
...
def add_decomposed_rel_pos(attn, q, rel_pos_h, rel_pos_w, q_size, k_size, token_num):
    q_h, q_w = q_size; k_h, k_w = k_size
    Rh = get_rel_pos(q_h, k_h, rel_pos_h); Rw = get_rel_pos(q_w, k_w, rel_pos_w)
    if token_num > 0: q = q[:, token_num:, :]
    B, _, dim = q.shape
    ...
    if token_num == 0:
        ...
        rel_h = rel_h.unsqueeze(-1).expand(B, -1, -1, -1, q_h).contiguous().view(B, attn.size(-2) - token_num, attn.size(-1) - token_num)
        rel_h = torch.cat((torch.zeros(B, token_num, rel_h.size(-1), device=attn.device), rel_h), dim=1)
        rel_h = torch.cat((torch.zeros(B, rel_h.size(-2), token_num, device=attn.device), rel_h), dim=-1)
        rel_w = rel_w.unsqueeze(-2).expand(B, -1, -1, q_w, -1).contiguous().view(B, attn.size(-2) - token_num, attn.size(-1) - token_num)
        rel_w = torch.cat((torch.zeros(B, token_num, rel_w.size(-1), device=attn.device), rel_w), dim=1)
        rel_w = torch.cat((torch.zeros(B, rel_w.size(-2), token_num, device=attn.device), rel_w), dim=-1)
...
class PatchEmbed(nn.Module):
    def __init__(self, kernel_size: Tuple[int, int] = (16, 16), stride: Tuple[int, int] = (16, 16),
                 padding: Tuple[int, int] = (0, 0), in_chans: int = 3, embed_dim: int = 768) -> None:
        super().__init__()
