...
class Sam(nn.Module):
    ...
    def __init__(self, image_encoder: ImageEncoderViT, prompt_encoder: PromptEncoder,
    ...
    def forward(self, batched_input: List[Dict[str, Any]], multimask_output: bool) -> List[Dict[str, torch.Tensor]]:
        ...
        for image_record, curr_embedding in zip(batched_input, image_embeddings):
            points = (image_record["point_coords"], image_record["point_labels"]) if image_record.get("point_coords") is not None else None
            sparse_embeddings, dense_embeddings = self.prompt_encoder(
                points=points, boxes=image_record.get("boxes"), masks=image_record.get("mask_inputs"))
