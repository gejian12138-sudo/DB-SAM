...
class Reshaper(nn.Module):
    def __init__(self, size_in: int, channel_in: int, size_out: int, channel_out: int,
        ...
        if size_in == size_out:
            self.reshaper = nn.ModuleList([nn.Conv2d(channel_in, channel_out, kernel_size=1)])
        elif size_in < size_out:
            n = int(math.log2(size_out // size_in))
            ...
            self.reshaper.extend([nn.Conv2d(channel_in, channel_out, kernel_size=1)])
        else:
            n = int(math.log2(size_in // size_out))
            self.reshaper = nn.ModuleList()
            for _ in range(n):
                self.reshaper.extend([
